...
module HMRC
  module ParsedResponse
    RSpec.describe EmploymentPayment do
      context 'valid hmrc response created via EmploymentIncomeSummary' do
        ...
        let(:employment_payment) { employment.payments.last }

        it 'returns the expected values' do
          expect(employment_payment.formatted_payment_date).to eq '2021-08-28'
          expect(employment_payment.gross_pay).to eq 2345.29
          expect(employment_payment.tax).to eq(-257.2)
          expect(employment_payment.national_insurance).to eq(-185.79)
          expect(employment_payment.benefits_in_kind).to eq 0.0
          expect(employment_payment.net_pay).to eq 1902.3
        end
      end
