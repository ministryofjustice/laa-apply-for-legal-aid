  ...
  describe ".call" do
    ...
    context "when the provider has ccms role" do
      ...
      context "and the provider cannot be found on Provider Details API" do
        before { allow(ProviderDetailsCreator).to receive(:call).and_raise(ProviderDetailsCWARetriever::ApiRecordNotFoundError) }
      ...
      context "and the provider details API not available" do
        before { allow(ProviderDetailsCreator).to receive(:call).and_raise(ProviderDetailsCWARetriever::ApiError) }
