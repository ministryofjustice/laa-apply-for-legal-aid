...
RSpec.describe Providers::TransactionsController do
  ...
  shared_examples_for "PATCH #providers/transactions" do
    it "saves the selected transactions" do
      expect { patch_request }.to change { bank_transaction.reload.transaction_type }.from(nil).to(transaction_type)
    end
  ...
  describe "updating transactions" do
    let!(:bank_transaction) { create(:bank_transaction, bank_account:, operation: transaction_type.operation) }
    let!(:bank_transaction_other_applicant) { create(:bank_transaction, operation: transaction_type.operation) }
    let(:selected_transactions) { [bank_transaction, bank_transaction_other_applicant] }
    let(:params) do
      {
    ...
    describe "PATCH #providers/incoming_transactions" do
      ...
      context "when being set to benefits" do
        ...
        let!(:selected_transactions) { [bank_transaction, bank_transaction_other_applicant, benefit_bank_transaction] }
    ...
    describe "PATCH #providers/outgoing_transactions" do
      ...
      context "when being set to rent" do
        ...
        let!(:selected_transactions) { [bank_transaction, bank_transaction_other_applicant, outgoing_bank_transaction] }
