...
RSpec.describe Providers::ApplicationMeritsTask::FirstAppealCourtTypesController do
  ...
  context "when not authenticated" do
    ...
    context "when GET /providers/:application_id/appeal_court_type" do
      before do
        get providers_legal_aid_application_first_appeal_court_type_path(legal_aid_application)
      end

      it_behaves_like "a provider not authenticated"
    end

    context "when PATCH /providers/:application_id/appeal_court_type" do
      before do
        patch providers_legal_aid_application_first_appeal_court_type_path(legal_aid_application)
      end
  ...
  describe "GET /providers/:application_id/appeal_court_type" do
    subject(:get_first_appeal_court_type) { get providers_legal_aid_application_first_appeal_court_type_path(legal_aid_application) }

    before { get_first_appeal_court_type }
    ...
    it "has expected path" do
      expect(page).to have_current_path(/.*\/appeal_court_type/, ignore_query: true)
    end

    it "displays the page" do
      expect(page)
        .to have_title("Which court will the appeal be heard in?")
        .and have_css("h1", text: "Which court will the appeal be heard in?")
    end
  end

  describe "PATCH /providers/:application_id/appeal_court_type" do
    subject(:post_first_appeal_court_type) { patch providers_legal_aid_application_first_appeal_court_type_path(legal_aid_application), params: }
    ...
    context "when form is saved using save and continue button" do
      ...
      context "when the user chooses a valid court type" do
        ...
        it "updates the appeal record with court_type" do
          expect { post_first_appeal_court_type }
