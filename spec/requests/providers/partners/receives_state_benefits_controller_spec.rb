...
RSpec.describe Providers::Partners::ReceivesStateBenefitsController do
  ...
  describe "GET /providers/applications/:legal_aid_application_id/means/receives_state_benefits" do
    subject(:make_request) { get providers_legal_aid_application_partners_receives_state_benefits_path(legal_aid_application) }
  ...
  describe "PATCH /providers/applications/:legal_aid_application_id/means/receives_state_benefits" do
    subject(:make_request) { patch(providers_legal_aid_application_partners_receives_state_benefits_path(legal_aid_application), params:) }

    let(:params) do
      { partner: { receives_state_benefits: } }
    end
    let(:setup) { nil }
    ...
    context "when the provider does not answer the question" do
      ...
      it "re-renders the page with an error" do
        expect(response).to have_http_status(:ok)
        expect(response.body).to include("Select yes if the partner gets any benefits")
      end
    end
