...
RSpec.describe Test::DatastorePayloadsController do
  ...
  describe "GET submit" do
    ...
    context "when provider authenticated" do
      ...
      context "when submission raises a ConnectionExpired error" do
        before do
          provider.entra_id_token.update!(expires_at: 1.minute.ago)
        ...
        it "redirects to session expired page" do
          expect(response)
            .to have_http_status(:redirect)
            .and redirect_to(session_expired_path(reason: :timeout))
        end
