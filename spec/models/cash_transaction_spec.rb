...
RSpec.describe CashTransaction, type: :model do
  ...
  def cash_transactions_for(application, multiplier)
    (1..3).each do |number|
      create :cash_transaction, transaction_type: benefits, legal_aid_application: application, transaction_date: number.month.ago, amount: 100 * multiplier
      create :cash_transaction, transaction_type: pension, legal_aid_application: application, transaction_date: number.month.ago, amount: 200 * multiplier
    end
  end
