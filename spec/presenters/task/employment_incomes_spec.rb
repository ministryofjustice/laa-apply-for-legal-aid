...
RSpec.describe Task::EmploymentIncomes do
  ...
  let(:application) { create(:legal_aid_application, applicant:, provider_received_citizen_consent:) }
  ...
  describe "#path" do
    ...
    context "when open banking consents is false" do
      ...
      context "when the provider has uploaded bank statement evidence" do
        ...
        context "when the applcant has multiple employments" do
        ...
        context "when the applcant has a single employment" do
          ...
          before{ Employment.create!(legal_aid_application: application, name: "employment", owner_id: applicant.id, owner_type: applicant.class) }
