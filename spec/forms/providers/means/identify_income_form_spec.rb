...
RSpec.describe Providers::Means::IdentifyIncomeForm do
  describe "validations" do
    context "when legal_aid_application_id is missing" do
      it "is invalid" do
        params = { "income_types" => [""] }
        ...
        expect(form.errors).to be_added(:legal_aid_application_id, :blank)
      end
    end

    context "when neither a income type or none are selected" do
      it "is invalid" do
        params = { "legal_aid_application_id" => "test-id", "income_types" => [""] }
    ...
    context "when at least one income type is selected, but an amount is missing" do
      it "is invalid" do
        ...
        params = {
          "legal_aid_application_id" => "test-id",
    ...
    context "when none is selected" do
      it "is valid" do
        params = { "legal_aid_application_id" => "test-id", "income_types" => ["", "none"] }
  ...
  describe "#save" do
    context "when the form is invalid" do
      ...
      it "does not update an application's regular payments" do
        legal_aid_application = create(:legal_aid_application)
        regular_payment = create(:regular_payment, legal_aid_application:)
        form = described_class.new(legal_aid_application_id: legal_aid_application.id)

        form.save

        expect(legal_aid_application.regular_payments).to contain_exactly(regular_payment)
      end
    end
    ...
    context "when none is selected" do
      ...
      it "does not create any regular payments" do
        legal_aid_application = create(:legal_aid_application)
        params = {
          legal_aid_application_id: legal_aid_application.id,
          income_types: ["", "none"],
        ...
        expect(RegularPayment.count).to eq 0
      end
      ...
      it "destroys any existing regular income payments" do
        ...
        expect(legal_aid_application.regular_payments).to contain_exactly(regular_outgoing_payment)
      end
    end
    ...
    context "when the correct attributes are provided" do
      ...
      it "updates an application's regular payments" do
        legal_aid_application = create(:legal_aid_application)
        benefits = create(:transaction_type, :benefits)
        pension = create(:transaction_type, :pension)
        ...
        regular_payments = legal_aid_application.regular_payments
        expect(regular_payments.count).to eq 2
        expect(regular_payments.pluck(:transaction_type_id, :amount, :frequency))
