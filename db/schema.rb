...
ActiveRecord::Schema.define(version: 20_210_126_151_112) do
  ...
  create_table 'actor_permissions', id: :uuid, default: -> { 'gen_random_uuid()' }, force: :cascade do |t|
    ...
    t.index %w[permittable_type permittable_id permission_id], name: 'actor_permissions_unqiueness', unique: true
    t.index %w[permittable_type permittable_id], name: 'index_actor_permissions_on_permittable_type_and_permittable_id'
  end
  ...
  create_table 'cash_transactions', id: :uuid, default: -> { 'gen_random_uuid()' }, force: :cascade do |t|
    ...
    t.uuid 'transaction_type_id'
    t.index %w[legal_aid_application_id transaction_type_id month_number], name: 'cash_transactions_unique', unique: true
  end
  ...
  create_table 'malware_scan_results', id: :uuid, default: -> { 'gen_random_uuid()' }, force: :cascade do |t|
    ...
    t.index %w[uploader_type uploader_id], name: 'index_malware_scan_results_on_uploader_type_and_uploader_id'
  end
  ...
  create_table 'proceeding_type_scope_limitations', id: :uuid, default: -> { 'gen_random_uuid()' }, force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[proceeding_type_id delegated_functions_default], name: 'index_proceedings_scopes_unique_delegated_default', unique: true,
                                                                where: '(delegated_functions_default = true)'
    t.index %w[proceeding_type_id scope_limitation_id], name: 'index_proceedings_scopes_unique_on_ids', unique: true
    t.index %w[proceeding_type_id substantive_default], name: 'index_proceedings_scopes_unique_substantive_default', unique: true, where: '(substantive_default = true)'
