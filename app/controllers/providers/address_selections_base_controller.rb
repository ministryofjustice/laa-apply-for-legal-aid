  ...
  private
    ...
    def address_selection_form_params
      merge_with_model(address, addresses: @addresses) do
        params.expect(address_selection: %i[lookup_id postcode]).merge(location:)
      end
    end

    def address_form_params
      merge_with_model(address) do
        params.expect(address_selection: %i[address_line_one address_line_two city county postcode lookup_postcode]).merge(location:)
      end
    end
  end
end
