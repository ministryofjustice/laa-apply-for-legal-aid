module Providers
  module LinkApplication
    class MakeLinksController < ProviderBaseController
      ...
      def show
        @form = Providers::LinkApplication::MakeLinkForm.new(model: lead_linked_application)
      end
    ...
    private
      ...
      def form_params
        merge_with_model(lead_linked_application) do
          next {} unless params[:legal_aid_application]

          params.require(:legal_aid_application).permit(:link_type_code)
        end
      end

      def lead_linked_application
