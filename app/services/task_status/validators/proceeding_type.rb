module TaskStatus
  module Validators
    ...
    private
      ...
      # It is not the job of the task list to verify the actual code and description are valid values
      ...
      # It is not the job of the task list to verify the actual service levels or scope limitations were set correctly by the form so we only
      # check that the emergency defaults were accepted or rejected.
      # We don't check the validity of the hearing date as that would require an LFA query
      # We don't check the validity of the limitation_note as the form does not (BUG?!) and
      # would also require an LFA query
      #
      # TODO: we do need to check the presence of hearing date and or limitation note to avoid users seeing a potentially complete
      # task list when they have not actually not completed the hearing date and.or limitation note, by using save and comabck later
      #
      def emergency_defaults_valid?
        return true if !used_delegated_functions? || special_children_act?
      ...
      def emergency_final_hearings_form
        ...
        # We do not need to check the validity of final hearing if one does not exist. however
        # it is possible that a user could have left the final hearing form blank with save and
        # come back later and the form did not create a final hearing record.
        #
      ...
      def emergency_scope_limitations_valid?
        ...
        # TODO: we need to add additional_params to scope_limitation model and save this value via the form when the user is
        # adding scopes. This can then be passed back to the form here to validate the existing scope limitation again.
