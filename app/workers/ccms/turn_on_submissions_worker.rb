  ...
  class TurnOnSubmissionsWorker
    ...
    def perform
      ...
      if applications.empty?
        ...
        Rails.logger.info "CCMS::RestartSubmissionsWorker - #{applications.size} submissions to restart, expected to take #{Time.zone.at(applications.size * 5).gmtime.strftime('%T')}"
        applications.each_with_index do |application, iteration|
          RestartSubmissionWorker.perform_in(iteration * 5, application.id)
        end
