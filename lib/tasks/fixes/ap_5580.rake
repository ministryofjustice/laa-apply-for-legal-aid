...
def can_be_migrated?(application)
  all_proceeding_states(application).uniq.count == 1 && uniq_proceeding_relationships(application).uniq.count == 1
end
  ...
  task :ap_5580, [:dry_run] => :environment do |_task, args|
    ...
    paa.each do |legal_aid_application|
      ...
      output_hash = {
        ...
        proceedings: count,
        proceeding_states: all_proceeding_states(legal_aid_application).uniq,
        proceeding_answers: uniq_proceeding_relationships(legal_aid_application).uniq,
      }
      ...
      unless dry_run
        if can_be_migrated?(legal_aid_application) == false
          ...
          ActiveRecord::Base.transaction do
            ...
            new_task.mark_as_complete! if all_proceeding_states(legal_aid_application).uniq == [:complete]
