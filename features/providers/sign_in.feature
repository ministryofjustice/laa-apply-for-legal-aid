Feature: Sign in
  ...
  @javascript @mock_auth_enabled @stub_offices_addresses_retriever @vcr_turned_off @stub_bank_holidays
  ...
  @javascript @mock_auth_enabled @mock_auth_array_is_set @vcr_turned_off @stub_bank_holidays
  Scenario: I am able to sign in and select an office when mock auth is enabled and mock_auth_array is set
    When I visit the root page
    Then I should see "Providers can use this service to apply for civil legal aid for their clients"
    ...
    When I fill 'email' with 'test1@example.com'
